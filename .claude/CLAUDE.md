...
| `VAPID_PRIVATE_KEY` | VAPID private key for web push (generate with `npx web-push generate-vapid-keys`) |
| `VAPID_SUBJECT` | VAPID subject (e.g. `mailto:admin@goalforge.app`) |
| `VITE_VAPID_PUBLIC_KEY` | VAPID public key — baked into frontend at build time; requires `docker compose build web` after change |
...
| `deps.py` | Shared FastAPI helpers — `_ensure_owner()`, `_load_user_with_ownership()`, `_load_goal_with_ownership()`, `_load_reward_with_ownership()`. Import from here in any new route; do NOT redefine inline. |
...
| `services/task_service.py` | Task completion, background sprint pre-gen, `create_sprint_tasks(db, goal_id, milestone_id, task_outputs, start_date)` — use for any new sprint task write loop; `compute_adaptive_difficulty_mode(goal_id, user_id, db)` — classifies 14-day completion rate into `lighter|balanced|stretch` |
...
| `routes/push.py` | Web push subscription CRUD (list, create, deactivate) |
| `services/push_service.py` | `send_push_to_user(user_id, title, body, db)` — delivers web push via `pywebpush`; skips silently if no active subscriptions |
| `ai_utils.py` | `generate_smart_goal()`, `generate_sprint_tasks()`, `regenerate_single_task()`, `resize_task_for_low_energy()`, `generate_star_log_narrative()`, `_with_retry()` — Gemini structured-output calls with retry + timeout |
| `exceptions.py` | `AIGenerationError` — raised by `_with_retry()` on final failure |
| `models.py` | SQLAlchemy ORM: `User`, `Goal`, `Milestone`, `DailyTask`, `Reward`, `WebPushSubscription`, `WeeklyReflection` |
...
| `POST` | `/goals/{goal_id}/tasks` | Add custom task to active sprint (auto-assigns milestone + today, timezone-aware via `user_today()`) |
| `PUT` | `/goals/{goal_id}/tasks/reorder` | Bulk-update task positions |
| `POST` | `/tasks/{task_id}/regenerate` | AI-regenerate a task's description + tip via Gemini (10/min per-user rate limit) |
...
| `POST` | `/users/{user_id}/weekly-reflection` | Submit weekly reflection + get AI coach recommendation (3/day rate limit); falls back to hardcoded advice on AI failure |
| `GET` | `/users/{user_id}/weekly-reflection/latest` | Get most recent weekly reflection |
| `GET` | `/users/{user_id}/weekly-review` | Weekly review metrics (completion rate, risk level, recommendation); `?days=` param (3-14, default 7) |
...
| `GET` | `/users/{user_id}/push-subscriptions` | List web push subscriptions for user |
| `POST` | `/users/{user_id}/push-subscriptions` | Register/upsert a web push subscription (dedup by endpoint) |
| `DELETE` | `/push-subscriptions/{subscription_id}` | Deactivate a push subscription (soft delete) |
| `GET` | `/users/{user_id}/star-log` | 7-day star log narrative (lazy compute & cache); `?days=` param (3-14, default 7); generates AI narrative via Gemini or fallback, persists to `star_logs` table, returns cached on repeat calls (10/hour) |
...
weekly_reflections  id (uuid), user_id → users, went_well (text), blockers (text),
               week_rating (int, 1-5 check), coach_recommendation (text), created_at
web_push_subscriptions  id (uuid), user_id → users, endpoint (text, unique), p256dh, auth,
               is_active (bool, default true), created_at, last_used_at (nullable)
star_logs      id (uuid), user_id → users, start_date, end_date, completed_tasks,
               completed_days, chapter_title (varchar 200), chapter_body (text),
               highlights (JSON list[str]), is_fallback (bool), created_at;
               UniqueConstraint(user_id, start_date, end_date) for idempotent caching
...
| `src/hooks/useWeeklyReview.ts` | `useWeeklyReviewQuery(userId, days)` — Analytics page weekly review metrics card |
| `src/hooks/useStarLog.ts` | `useStarLogQuery(userId, days)` — Analytics page star log narrative card |
| `src/hooks/useWeeklyReflection.ts` | `useLatestWeeklyReflectionQuery(userId)` + `useCreateWeeklyReflectionMutation(userId)` — Analytics page weekly reflection |
| `src/hooks/usePushNotifications.ts` | `usePushNotifications(userId)` — SW registration, VAPID subscribe, server sync |
...
| `public/sw.js` | Service worker — push notifications only, NO fetch caching (see comment in file) |
| `public/manifest.webmanifest` | PWA manifest — linked from `index.html` |
...
| `src/lib/types.ts` | Shared TypeScript interfaces: `Goal` (incl. `rescue_mode`), `Milestone`, `Task` (incl. `is_rescue_task`, `original_description`, `original_tip`), `UserSettings`, `UserSettingsUpdatePayload`, `PaginatedGoalsResponse`, `Reward`, `RewardDrop`, `RewardTier`, `TaskCompleteResponse`, `EnergyResizeResponse` |
...
- **Rate limiting** (`slowapi`): Global default 100 req/min per IP. Stricter per-user limits: `POST /users/{user_id}/goals` → 5/min; `POST /goals/{goal_id}/milestones/{milestone_id}/complete` → 10/min; `POST /tasks/{task_id}/regenerate` → 10/min. Exceeded requests get 429 with `Retry-After: 60`. Disable entirely with `RATE_LIMIT_ENABLED=false` (tests, local dev). Rate-limited endpoints require `request: Request` as their first parameter. The `rate_limit()` helper in `main.py` is a no-op when disabled — no middleware or exception handler is registered.
- **Strict Boundaries**:
...
- **`git rm --cached` + merge deletes working tree files**: Untracking gitignored files with `git rm --cached` and merging the commit will delete those files from the working tree on `git pull`. Back up local-only files (like `.claude/CLAUDE.md`) before merging such changes.
...
- **Adaptive sprint difficulty**: `compute_adaptive_difficulty_mode(goal_id, user_id, db)` in `services/task_service.py` classifies recent 14-day completion into `lighter|balanced|stretch` and passes `difficulty_mode` into `generate_sprint_tasks(...)` from both milestone routes and background pre-gen. No DB column — computed at runtime from existing `DailyTask.is_completed` data.
- **Coworker AI agent**: `.claude/COWORKER.md` contains the memory/CLAUDE.md of an autonomous AI agent that bulk-generated features. Compare against CLAUDE.md when reviewing coworker PRs for divergence. PRs from this agent often need rebasing — they branch from old base and may contain already-merged commits.
