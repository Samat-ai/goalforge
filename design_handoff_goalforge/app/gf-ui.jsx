// gf-ui.jsx — shared primitives for the GoalForge redesign.
// Exports to window: Icon, Ring, Mascot, useCountUp, useInView, Reveal,
//   Sparkline, Donut, Segmented, Flame, cx.
...
const GOAL_ICON = { fitness: 'run', career: 'bolt', learning: 'book', wellness: 'heart' };

// ── Hooks ──────────────────────────────────────────────────────────────────────
// Count-up number animation.
function useCountUp(target, { duration = 1000, decimals = 0, start = true, fromRatio = 0.82 } = {}) {
  ...
  const f = decimals > 0 ? val.toFixed(decimals) : Math.round(val).toLocaleString();
  return f;
}
...
function Reveal({ children, delay = 0, y = 14, className, style, as = 'div' }) {
  const El = as;
  // After the first tab switch, the PageSwitcher wrapper owns the entrance
  ...
  if (typeof window !== 'undefined' && window.__gfRevealOff) {
...
function Switcher({ value, scrollTop = false, className, children }) {
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      setShown(value);
      setPhase('in');
      if (scrollTop) { try { window.scrollTo({ top: 0, behavior: 'instant' }); } catch (e) { window.scrollTo(0, 0); } }
    }, 160);
...
function Ring({ value, size = 120, stroke = 12, color = 'var(--accent)', track = 'var(--ring-track)', children, delay = 120, rounded = true, gradientId, fromRatio = 0 }) {
...
// ── Flame streak chip ────────────────────────────────────────────────────────────
function Flame({ n, size = 13, color = 'var(--accent)', muted }) {
  return (
...
function Mascot({ stageId = 3, size = 96, mood = 'happy' }) {
...
function Segmented({ options, value, onChange, getLabel }) {
  ...
  useEffect(() => {
    const el = wrap.current?.querySelector(`[data-seg="${value}"]`);
...
Object.assign(window, { Icon, GOAL_ICON, Ring, Mascot, Flame, Sparkline, Segmented, Toggle, Switcher, useCountUp, useInView, Reveal, cx, gfTip, gfHideTip });

// ── Custom floating tooltip (replaces ugly native title rectangles) ──────────────
function gfTip(e, html) {
