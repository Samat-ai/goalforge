...
test('core loop: login, create goal, complete 3 tasks, and redeem shop reward', async ({ page }) => {
  ...
  function makeGoal(rawInput: string): GoalState {
    const goalId = 'goal-1'
  ...
  await page.route('**/*', async route => {
    ...
    if (method === 'POST' && pathname === `/users/${userId}/goals`) {
      ...
      const goal = makeGoal(payload.raw_input ?? 'Untitled')
      state.goals = [goal]
      await route.fulfill({ status: 200, headers: jsonHeaders(), body: JSON.stringify(goal) })
      return
    }
  ...
  await expect(page.getByLabel('Mark task complete').first()).toBeVisible()
